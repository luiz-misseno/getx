...
extension GetRestorableNavigation on GetInterface {
  ...
  String? restorableTo<T>(
    ...
    return searchDelegate(id).navigatorKey.currentState?.restorablePush<T>(
  ...
  String? restorableToNamed<T>(
    ...
    return searchDelegate(id).navigatorKey.currentState?.restorablePushNamed<T>(
          page,
          arguments: arguments,
        );
  }
...
GetPageBuilder _resolve(dynamic page, String method) {
  if (page is GetPageBuilder) {
    ...
    Get.log('''WARNING, consider using: "Get.$method(() => Page())"
instead of "Get.$method(Page())".
 Using a widget function instead of a widget fully guarantees that the widget
and its controllers will be removed from memory when they are no longer used.
       ''');
